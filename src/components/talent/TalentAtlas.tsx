...
import { PROFICIENCY_EVIDENCE, TALENT_EVIDENCE } from '../../data/talent/evidence';
...
const TalentAtlas: React.FC<{ own?: boolean; baseRoute?: string }> = ({ own = false, baseRoute = '/talent-tree/atlas' }) => {
  ...
  const pools = atlasPools(own ? effectiveIntake(build.intake, answered) : build.intake, build.craftCredit, build.priorPracticeYears);
  ...
  const evidence = !own ? PROFICIENCY_EVIDENCE[abilityId] : undefined;
  ...
  const changeIntake = (patch: Partial<Intake>, answer?: IntakeQuestion) => {
    ...
    setBuild({ ...build, intake, allocation: clampAtlas(build.allocation, atlasPools(effectiveIntake(intake, nextAnswered), build.craftCredit, build.priorPracticeYears)) });
  ...
  return <main className="atlas forge" id="main-content">
    ...
    <section className="forge-inspector" ref={inspectorRef} tabIndex={-1} aria-label="Proficiency details">
      ...
      {known && evidence && <p className="forge-evidence"><Link to={evidence.href}>{evidence.title} <span aria-hidden="true">↗</span></Link><span>{evidence.detail}</span></p>}
    ...
    <details className="forge-rules"><summary>How your class takes shape</summary><p>Your talents unlock passive proficiencies. These shape six mastery domains: Form, Meaning, Behavior, Structure, Realization, and Stewardship. Your strongest domain determines your class unless you qualify for a combined class. Browsing never changes it.</p><p>Each domain has three facets. Only the strongest active proficiency in each facet counts, up to five points each. Total strength decides your class; broader coverage breaks ties, followed by the fixed domain order shown below.</p><p>Craft Steward combines visual craft (Pixel Prowess or Brand Barrage), durable standards (Durable Standards), and creative exploration (Tangible Exploration or Prototype Pulse). When all three pillars are active, this combined class takes precedence. Only the strongest proficiency in each pillar counts; the weakest pillar sets the class level.</p><p>Domain class ranks follow average facet strength, with Initiate for a single facet, at most Apprentice for two, and Expert until all three reach five for Master. These are build ranks, not professional certifications. A proficiency’s own rank follows its weakest ingredient.</p><div className="forge-domain-list">{mastery.domains.map(domain => <div key={domain.id}><ForgeMark id={domain.id}/><strong>{domain.name}</strong><span>{domain.strength}/15</span><p>{domain.facets.map(f => `${f.name}: ${f.rank}/5`).join(' · ')}</p></div>)}</div></details>
