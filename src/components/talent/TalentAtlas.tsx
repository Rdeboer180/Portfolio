...
import { CLASS_LEVELS, proficiencyName, resolveMastery } from '../../data/talent/mastery';
...
const TalentAtlas: React.FC<{ own?: boolean; baseRoute?: string }> = ({ own = false, baseRoute = '/talent-tree/atlas' }) => {
  ...
  const [showAll, setShowAll] = useState(own);
  const [lane, setLane] = useState<TreeId>('design');
  const [filter, setFilter] = useState<'earned' | 'all'>(own ? 'all' : 'earned');
  ...
  const earned = states.filter(s => s.rank > 0);
  ...
  const reset = () => { setBuild(EMPTY_BUILD); setAnswered(NOTHING_ANSWERED); setDirty(false); setCopied(false); setShowAll(true); setSkillId(null); setFilter('all'); const url = new URL(window.location.href); url.hash = ''; window.history.replaceState(null, '', url.toString()); setAnnouncement('Build reset. Your class is Initiate Maker.'); };
  ...
  return <main className="atlas forge" id="main-content">
    ...
    <div className="forge-section-heading"><h2>At the workbench</h2><span>Talents → proficiencies → class</span></div>
    ...
    <div className="forge-workbench-controls"><p>{own ? `${pools.total - spent} points available${!live ? ' · Complete your experience above to invest' : ''}` : 'Explore the talents behind this proficiency'}</p><button aria-pressed={showAll} onClick={() => setShowAll(!showAll)}>{showAll ? 'Focus recipe' : 'Show all 32 talents'}</button></div>
    ...
    {known && <div className="forge-convergence"><svg viewBox="0 0 900 80" preserveAspectRatio="none" aria-hidden="true">{TREES.map((tree, i) => recipe.some(id => ATLAS_SKILL_BY_ID[id].territory === tree.id) && <path key={tree.id} className={`atlas-territory--${tree.id}`} d={`M${150 + i * 300} 0 C${150 + i * 300} 50 450 25 450 80`}/>)}</svg><span className="forge-medallion"><ForgeMark id={abilityId} size={28}/></span><strong>{proficiencyName(selected)}</strong><small>{selected.rank > 0 ? 'Active passive' : 'Currently inactive'} · {selected.rankLabel}</small></div>}
    <section className="forge-collection" aria-label="Proficiency collection"><div className="forge-section-heading"><div><h2>Your proficiencies</h2><p>Passive strengths, earned through practice.</p></div><div className="forge-filters"><button aria-pressed={filter === 'earned'} onClick={() => { setFilter('earned'); setExpandedCollection(false); }}>Active · {earned.length}</button><button aria-pressed={filter === 'all'} onClick={() => { setFilter('all'); setExpandedCollection(false); }}>All · 31</button></div></div><div className="forge-collection__grid">{states.filter(s => filter === 'all' || s.rank > 0).slice(0, expandedCollection ? 31 : 6).map((s, i) => <button key={s.ability.id} aria-pressed={s.ability.id === abilityId} aria-label={s.visibility === 'discovered' ? `${proficiencyName(s)}. ${s.rankLabel}.` : `Undiscovered proficiency ${i + 1}. ${s.activeConnections} connections detected.`} className={s.visibility !== 'discovered' ? 'is-unknown' : ''} onClick={() => inspect(s.ability.id)}><span className="forge-medallion">{s.visibility === 'discovered' ? <ForgeMark id={s.ability.id}/> : '?'}</span><strong>{s.visibility === 'discovered' ? proficiencyName(s) : '???'}</strong><small>{s.visibility === 'discovered' ? `${s.rankLabel} · ${s.rank}/5` : s.visibility === 'resonating' ? 'Resonating' : 'Undiscovered'}</small></button>)}</div>{(filter === 'all' ? 31 : earned.length) > 6 && <button className="forge-link" onClick={() => setExpandedCollection(!expandedCollection)}>{expandedCollection ? 'Show fewer proficiencies' : `Show all ${filter === 'all' ? 31 : earned.length} proficiencies`}</button>}{filter === 'earned' && !earned.length && <p>No active proficiencies yet. Invest in talents to discover your first connection.</p>}</section>
    ...
    <details className="forge-rules"><summary>Your point receipt</summary><p>Experience sets the budget. Each talent accepts up to five points. Advanced talents retain their prerequisites. Removing a prerequisite also removes investments that depend on it.</p><ul>{pools.receipt.map((line, i) => <li key={i}>{line.label}</li>)}</ul><p>{spent} of {pools.total} points invested. Some education points are reserved for their corresponding tree.</p></details>
