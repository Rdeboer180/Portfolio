...
### Ecosystem Node
The card used on the homepage's How I work board (section 03) (and documented live under Molecules → Ecosystem Node). One shape, three tiers, and the tier is carried by structure rather than by a category colour:
- **Hub** (Shared Product Context): paper, 1px Ink border, and the `selection-frame` corner handles. It reads as the primary object on the board without taking a hue.
